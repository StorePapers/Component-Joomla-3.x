...
class StorepapersControllerBorrarAutor extends StorepapersControllerDefault{	
	...
	*/		
	public function remove(){	
		...
		$ida    = JRequest::getCmd('id');	
		
		$row = $modelo->getAutor($ida);
		$res = $modelo->borrarAutor($ida);		
		
		if($res == 1)
			$this->setMessage(JText::sprintf('EXITO_BORRAR_AUTOR', $row[0]->nombre));			
        else
			$this->setMessage(JText::sprintf('ERROR_BORRAR_AUTOR', $row[0]->nombre));        
