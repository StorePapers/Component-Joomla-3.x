...
class StorepapersControllerInsertarNuevaCategoria extends StorepapersControllerDefault{	
	...
	*/	
	public function save($cachable = false){
		...
		//En este caso no quiero utilizar este modelo sino el de editar categoria
		//porque utilizo la misma operación
		$modelo =& $this->getModel('EditarCategoria');
		
		$res = $modelo->guardarCategoria(JRequest::get('post',JREQUEST_ALLOWHTML));		
		
		if($res == 1)
			$this->setMessage(JText::_('EXITO_INSERTAR_MODIFICAR_CATEGORIA'));
        else
			$this->setMessage(JText::_('ERROR_INSERTAR_MODIFICAR_CATEGORIA'));			
        
		$this->setRedirect('index.php?option='.$option.'&view=mostrarCategoria');	
		$this->redirect();
	}	
