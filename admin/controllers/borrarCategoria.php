...
class StorepapersControllerBorrarCategoria extends StorepapersControllerDefault{	
	
	/*
	Función que se ejecuta cuando una vez confirmado el autor a borrar.
	*/		
	public function remove(){	
		...
		$idc    = JRequest::getCmd('id');	
		
		$row = $modelo->getCategoria($idc);
		$res = $modelo->borrarCategoria($idc);		
		
		if($res == 1)
			$this->setMessage(JText::sprintf('EXITO_BORRAR_CATEGORIA', $row[0]->nombre));			
        else
			$this->setMessage(JText::sprintf('ERROR_BORRAR_CATEGORIA', $row[0]->nombre));        
