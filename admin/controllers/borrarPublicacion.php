...
class StorepapersControllerBorrarPublicacion extends StorepapersControllerDefault{	
	
	/*
	Función que se ejecuta cuando una vez confirmado la publicación a borrar.
	*/		
	public function remove(){
		...
		$idp    = JRequest::getCmd('id');		
		
		$row = $modelo->getPublicacion($idp);
		$res = $modelo->borrarPublicacion($idp);
		
		if($res == 1)
			$this->setMessage(JText::sprintf('EXITO_BORRAR_PUBLICACION', $row[0]->nombre));			
        else
			$this->setMessage(JText::sprintf('ERROR_BORRAR_PUBLICACION', $row[0]->nombre));        
