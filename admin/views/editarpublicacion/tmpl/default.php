...
<form name="adminForm" id="adminForm" action="index.php" method="post">
	...
	<div class="form-inline form-inline-header">
		<div class="control-group">
			...
			<div class="controls">
				<input id="jform_title" class="input-xxlarge input-large-text required" type="text" MAXLENGTH="250" size="60" value="<?php echo $this->data['publicacion'][0]->nombre;?>" name="nombre" aria-required="true" required="required">
			</div>
		</div>
	</div>
	...
	<div class="row-fluid">
		<div class="span9">
			...
			echo $editor->display('texto', $this->data['publicacion'][0]->texto,'100%','400','100','6',true);
			?>
		</div>
		<div class="span3">
			<div class="control-group">
				...
				<div class="controls">
					<select id="jform_state" class="inputbox" size="1" name="year">
						...
						for ($i = $this->data['config'][0]->first; $i < $n; $i++){			
							if($i == $this->data['publicacion'][0]->year)
			...
			<div class="control-group">
				...
				<div class="controls">
					<select id="jform_state" class="inputbox" size="1" name="month">
						<?php
						for ($i = 1; $i <= 12; $i++){
							if($i == $this->data['publicacion'][0]->month)
				...
				<div class="controls">
					<select id="jform_state" class="inputbox" size="1" name="idc">
						...
						for ($i=0; $i < $n; $i++){
							...
							if($row->id == $this->data['publicacion'][0]->idc)
					...
					<table border="0" cellspacing="0" cellpadding="0" align ="center">
						<tbody>
							...
							for ($i=0; $i < $n; $i++){
								...
								if($enc == 1)
									echo '<tr><td>'.$row->nombre.'</td><td><INPUT TYPE="TEXT" NAME="prioridad_id'.$row->id.'" MAXLENGTH=3 SIZE="3" VALUE="'.$prioridad.'">';	
								else
	...
	<input type="hidden" name="view"   		id="view"   	value="editarPublicacion" />
