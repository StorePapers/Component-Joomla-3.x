...
class StorepapersViewBorrarPublicacion extends JViewLegacy{	
	...
	function display($tpl = null){
		...
		//Leo la publicacion en concreto a borrar
		$data['publicacion'] = $model->getPublicacion($idp);		
		$data['autoresPublicacion'] = $model->getAutoresSegunPublicacion($idp);
		
		//Leo la categoria y los autores de la BD
		$data['categoria'] = $model->getCategoria($data['publicacion'][0]->idc);
		$data['autores'] = $model->getAutores();
		
		$this->assignRef( 'data', $data );
		
		//Añado la barra de herramientas
		$this->addToolbar($ida, $idc, $year);
		
		parent::display($tpl);		
	}
	
	protected function addToolbar($ida, $idc, $year) {
		...
		// Añade el titulo en la barra de herramientas
		JToolBarHelper::title(JText::_('COM_STOREPAPERS').': '.JText::_('BORRAR_PUBLICACION'),'storepapers');
		JHtml::stylesheet('com_storepapers/backend.css', array(), true, false, false);
